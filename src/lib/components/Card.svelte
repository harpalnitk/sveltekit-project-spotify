	...
	.card {
        ...
        h4 {
			...
			a {
                ...
                // for placing focus ring on whole crad
                //instead of just the anchor tag using
                //pseudo element on anchor tag
				&:focus-visible {
					outline: none;
					&:after {
                ...
                //pseudo element used to position anchor tag
                //on entire card so that clicking on
                //anywhere in the card will click on anchor tag
				&:after {
					content: '';
					position: absolute;
					top: 0;
					...
					right: 0;
					bottom: 0;
                    // same border radius as card; used for outline
					border-radius: 4px;
				}
			}
		}
		p {
			position: relative;// for selecting text
            // by giving position relative p tag lives on top of
            // pseudo anchor tag
			margin: 0;
		...
		img {
			width: 100%;
			aspect-ratio: 1; // all of them will be square ; if they come as rectangle from server
			object-fit: cover;
			margin: 0 0 20px;
		}
		.cover-placeholder {
			...
			margin: 0 0 20px;
			transition: background 0.3s;
            // icon will scale based on size of viewport
            //by below code
			:global(svg) {
				width: 40%;
				height: 40%;
